...
namespace MyVieShowApp
{
    public partial class MainWindow : Form
    {
        ...
        private void submitButton_Click(object sender, EventArgs e)
        {
            ...
            {
                for (int number = 0; number <= seatAvail.Length - numberOfPeople; number++)
                {
                    if (!seatAvail[number])
                        continue;
                    ...
                    {
                        if (seatAvail[next] && next / numberOfRow == number / numberOfRow)
                            finded++;
                        else
                            break;
                    }
                    ...
                    if (finded == numberOfPeople)
                    {
                        ...
                        {
                            seats[number].Image = Properties.Resources.seat_select;
                            string receipt = "您選擇的座位是:\n";
                            receipt += string.Format("第{0}排{1}號", index / numberOfRow, index % numberOfRow + 1);
                        }
                        RichTextBox.Text = receipt
                        break;
                    }
                }
            }
